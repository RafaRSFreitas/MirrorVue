...
- **Setting controls** - Allows the user to control the landscape rotation and the keep-screen awake.
- **Eye Icon** - Show or hide the controls.
...
- Node.js 20.19.4+
- JDK 17 — set `JAVA_HOME` to a JDK 17 installation (e.g. `C:\Program Files\Java\jdk-17`); Android Studio bundles one under `jbr/` so this is usually satisfied if you build from the IDE, but CLI builds require it explicitly
- Android Studio with a configured emulator or a physical Android device (API 26+)
- Android SDK — `expo run:android` needs the Android SDK; either set the `ANDROID_HOME` environment variable or ensure `android/local.properties` contains `sdk.dir=C:\\<your-sdk-path>`
...
# Generate the native Android project (--clean regenerates from scratch, avoiding stale native folders)
...
> **Note:** `android/` is generated by `prebuild` and is gitignored. On a fresh clone, `expo run:android` requires the Android SDK to be discoverable (`ANDROID_HOME` env var or `android/local.properties`), and the Gradle wrapper (Gradle 9.3.1) requires JDK 17+.
